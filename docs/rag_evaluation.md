...
- 최초 검색 평가일: 2026-07-24
- LangGraph/LLM 통합 재검증일: 2026-07-26
...
검색된 Top-3 Chunk를 LangGraph와 Gemini LLM에 전달해 실제 CLI 답변도
확인했다. 답변 모델은 `GEMINI_LLM_MODEL=gemini-3.6-flash`를 사용했다.

| 질문 | 실제 CLI 결과 | 판정 |
|---|---|---|
| AI컴퓨터전자공학부 4학년 2학기에는 어떤 과목이 있나요? | 관련 4학년 2학기 과목을 자연어로 안내 | PASS |
| 머신러닝 과목의 선수과목은 무엇인가요? | Calculus 2, 선형대수학 | PASS |
| 캡스톤디자인 2는 몇 학점이며 선수과목은 무엇인가요? | 4학점, 캡스톤디자인 1 | PASS |
| AI컴퓨터전자공학부 건물의 주차장은 몇 층인가요? | 제공된 문서에서 확인할 수 없다는 fallback | PASS |

기존 `gemini-2.5-flash`는 실제 실행 시 신규 사용자에게 제공되지 않아
404를 반환했다. 현재 모델은 코드에 고정하지 않고 `GEMINI_LLM_MODEL`
환경변수로 설정해 모델 수명 주기 변경에 대응하도록 했다.
...
.venv/bin/python app.py "머신러닝 과목의 선수과목은 무엇인가요?"
```
...
- 실제 통합 검증은 위 네 질문에 한정되므로 더 다양한 질문으로 회귀 테스트해야 한다.
